module const
    ...
    integer, parameter :: nomp = 4 ! number of threads for OMP execution
...
subroutine transmfunc(VL, E, P, T, R)
    use const
  	...
  	real(8)                                         :: dx, kLz, Tk
    integer                                         :: i, j, ikx, iky
    integer                                         :: nthreads, threadid, chunk
    ...
    !$OMP PARALLEL PRIVATE (H, b, info, ipiv, kLz, kRz, transm, reflec, T, R) SHARED (Tk)
    ...
    if (abs(real(kL)) > 0.0 .and. abs(real(kR)) > 1.0E-9) then 
        ...
        do ikx = 1, floor(sqrt(2 * me * (lx ** 2) * (E - VL) / (pi * hbar) ** 2))
            chunk = floor(sqrt(2 * me * (ly ** 2) * (E - VL) / (pi * hbar) ** 2 - (ikx * ly / lx) ** 2)) / nomp
